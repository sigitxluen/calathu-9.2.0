...
unsigned long time_passed = 0;
  ...
  if (time_passed > 3000)
  {
    ...
    time_passed = 0;
    Serial.println("beat");
    // SerialMurata.println("#1=36-32-31-39-77-39-7F-18/2=00EEC8E6/3=0/4=21/5=21/6=17/7=17/8=0/9=0/");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Temp = "); //Dht[i].t, Dht[i].h
    lcd.setCursor(0, 1);
    lcd.print("Humi = "); //Dht[i].t, Dht[i].h
    lcd.setCursor(8, 0);
    lcd.print(TasmotaGlobal.temperature_celsius); //Dht[i].t, Dht[i].h
    lcd.setCursor(8, 1);
    lcd.print(TasmotaGlobal.humidity); //Dht[i].t, Dht[i].h
    lcd.setCursor(13, 0);
    lcd.print(" C"); //Dht[i].t, Dht[i].h
    lcd.setCursor(13, 1);
    lcd.print(" %"); //Dht[i].t, Dht[i].h
